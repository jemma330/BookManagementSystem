...
function buttonClick(obj){
	var a = document.getElementById('rentId');
	a.value=obj.id;
}
</script>
	<!-- 引入jquery -->
	...
	<div style="padding-left: 300px; padding-right: 300px">
		<form action="${pageContext.request.contextPath }/cancelorder.do">
			<input type="text" id="rentId" name="rentId" hidden="hidden"/>
			<table class="table table-hover">
				...
				<c:forEach items="${bookorderlist }" var="book">
					<tr>
						<td>${book.bookid }</td>
						<td>${book.bookname }</td>
						...
						<td>${book.enddate }</td>
						<td>
							<button id="${book.rentid }" onclick="buttonClick(this);" class="btn btn-primary" aria-label="Left Align">Cancel</button>
						</td>
					</tr>
				</c:forEach>
			</table>
